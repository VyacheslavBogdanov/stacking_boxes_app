...
## - [x] Задача 10. E2E-тесты

**Цель:** полные пользовательские сценарии через Playwright.

**Файлы:**

- [x] `e2e/calculator.e2e.ts` — сценарий: заполнение формы → получение результата
- [x] `e2e/admin.e2e.ts` — сценарий: вход админа → CRUD марок картона → сохранение авторизации после обновления → logout
- [x] `e2e/history.e2e.ts` — сценарий: несколько расчётов → история последних 10 расчётов → просмотр записи → закрытие результата
- [x] `playwright.config.ts` — конфигурация Playwright, запуск frontend/backend для E2E, `testIdAttribute: 'data-test'`
...
- [x] `npm run test` проходит: 24 файла, 163 теста
- [x] `npm run test:e2e` проходит: 5 E2E-тестов
